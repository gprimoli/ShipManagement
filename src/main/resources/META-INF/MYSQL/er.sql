...
CREATE TABLE porto(
	localcode				varchar(6),
	nome					varchar(50),
    codice_area				varchar(10) REFERENCES area(codice) on UPDATE CASCADE on DELETE CASCADE,
...
CREATE TABLE imbarcazione (
	...
	posizione				varchar(10) REFERENCES area(codice) on UPDATE CASCADE on DELETE CASCADE,
...
CREATE TABLE richiesta (
    ...
    stato		 			enum('Disponibile', 'In Lavorazione', 'Terminata') NOT NULL,
    documento 				blob,
							PRIMARY KEY (id)
);
...
CREATE TABLE mediazione (
	id						bigint auto_increment,
    nome 					varchar(50) NOT NULL,
    stato 					enum('Default', 'In Corso', 'Richiesta Modifica', 'Richiesta Terminazione', 'In Attesa di Firma', 'Terminta') NOT NULL,
    contratto 				blob,
	cod_fiscale_utente 		varchar(16) REFERENCES utente(cod_fiscale) on UPDATE CASCADE on DELETE CASCADE,
							PRIMARY KEY (id)
);
...
CREATE TABLE mediazione_richiesta (
	...
	id_richiesta 			varchar(16) REFERENCES utente(id) on UPDATE CASCADE on DELETE CASCADE,
